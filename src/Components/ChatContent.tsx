...
export default function ChatContent() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4000/api/messages/${id}`)
      .then(response => {
        const data = Array.isArray(response.data) ? response.data : [];
        setMessages(data);
        const user = data.length > 0 ? data[0].user : null;
        setSelectedUser({ name: user, avata: 'https://images2.thanhnien.vn/528068263637045248/2024/1/25/e093e9cfc9027d6a142358d24d2ee350-65a11ac2af785880-17061562929701875684912.jpg' });
      })
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center">
            <div className="relative">
              <img
                src={selectedUser?.avata}
                alt={selectedUser?.name || "Người dùng"}
                className="relative w-[40px] h-[40px] rounded-full mr-3"
              />
              <div className="absolute top-[-5px] right-[10px]">
                <OnlineBtn />
            ...
            <div>
              <h2 className="text-lg font-semibold">
                {selectedUser?.name || "Không rõ người dùng"}
              </h2>
              <h3>Đang online 2 giờ</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 px-[8px] pt-[8px] overflow-y-auto rounded-content-right">
        {Array.isArray(messages) && messages.length > 0 ? (
          messages.map((msg, index) => (
            <div
              key={index}
              className={`flex mb-4 ${msg.is_sent ? "justify-end" : "justify-start"}`}
            >
              <div
                className={`flex flex-col ${msg.is_sent ? "items-end" : "items-start"} w-full`}
              >
                <div
                  className={`flex text-left items-center gap-1 w-full ${msg.is_sent ? "justify-end" : "justify-start"}`}
                >
                  {!msg.is_sent && (
                    <img
                      src={selectedUser?.avata}
                  ...
                  <div
                    className={`max-w-[50%] relative ${
                      msg.is_sent
                        ? "rounded-content-right bg-[#E0F0FF] text-[#080707]"
                        : "rounded-content-left bg-[#E9EAED] text-[#080707]"
                    } ${msg.is_sent ? "text-right" : "text-left"}`}
                  >
                    {selectedEmojis[index] && (
                      <div className="absolute right-0 bottom-[-12px] text-xl">
                        {selectedEmojis[index]}
                    ...
                    {msg.link && (
                      <a
                        ...
                        rel="noopener noreferrer"
                        className={`truncate p-2 max-w-[200px] block w-[150px] ${
                          msg.is_sent ? "text-blue-200" : "text-blue-500"
                        } underline`}
                      >
                        {msg.link}
                      </a>
