...
export default function Sidebar() {
  ...
  const [isSearchModalOpen, setIsSearchModalOpen] = useState(false);
  const [viewedChats, setViewedChats] = useState(new Set());
  const [chatList, setChatList] = useState([]); // Khởi tạo là mảng rỗng

  useEffect(() => {
    axios.get('http://localhost:4000/api/chat-list')
      .then(response => {
        // Đảm bảo chatList luôn là mảng
        const data = Array.isArray(response.data) ? response.data : [];
        setChatList(data);
      })
      .catch(error => {
        console.error('Lỗi khi lấy danh sách cuộc trò chuyện:', error);
        setChatList([]); // Gán mảng rỗng nếu có lỗi
      });
  }, []);
